...
export function createPersonalStatsController(useCase: QueryPersonalStats, deleteUseCase: DeleteUploadSession): Router {
  ...
  router.get('/:token/skipped', handle((t, f) => useCase.getSkippedTracks(t, f.limit ?? DEFAULT_TOP_LIMIT)));
  router.get('/:token/artist-loyalty', handle((t, f) => useCase.getArtistLoyalty(t, f)));
  router.get('/:token/back-button', handle((t, f) => useCase.getBackButtonTracks(t, f.limit ?? DEFAULT_TOP_LIMIT)));
  ...
  router.get('/:token/shuffle-serendipity', handle((t, f) => useCase.getShuffleSerendipity(t, f.limit ?? DEFAULT_TOP_LIMIT)));
  router.get('/:token/intro-test', handle((t, f) => useCase.getIntroTestTracks(t, f.limit ?? DEFAULT_TOP_LIMIT)));
  ...
  router.get('/:token/skip-graveyard', handle((t, f) => useCase.getSkipGraveyard(t, f.limit ?? DEFAULT_TOP_LIMIT)));
  router.get('/:token/seasonal-artists', handle((t) => useCase.getSeasonalArtists(t)));
  router.get('/:token/rebound-artists', handle((t, f) => useCase.getReboundArtists(t, f.limit ?? DEFAULT_TOP_LIMIT)));
