    ...
    <div class="chart-header">
      <div>
        ...
        <p class="section-subtitle">
          Cumulative listening hours — see exactly when each track took over
          your life.
        </p>
      </div>
      <div v-if="chartData && selectedTracks.length > 0" class="controls">
        <span class="selection-count"
          >{{ selectedTracks.length }} of {{ legendItems.length }} shown</span
        >
        ...
        <button
          ...
          :class="{
            'legend-item--active':
              ...
              selectedTracks.includes(item.label),
            'legend-item--dim':
              selectedTracks.length > 0 && !selectedTracks.includes(item.label),
          }"
          :style="
            selectedTracks.includes(item.label)
